...
class Data:
    ...
    def preprocessing(self, data_path_list, div, begin_offset, end_offset, out_path, okt_tokenizer, khaiii_api, kkma_tokenizer):
        self.div = div
        reader = Reader(data_path_list, div, begin_offset, end_offset)
        rets = []
        for pid, label, h, i in reader.generate():
            y, x = self.parse_data(label, h, i, okt_tokenizer, khaiii_api, kkma_tokenizer)
            if y is None:
                continue
            rets.append((pid, y, x))
    ...
    def parse_data(self, label, h, i, okt_tokenizer, khaiii_api, kkma_tokenizer):
    ...
    def copy_chunk(self, dataset, chunk, offset, with_pid_field=True):
        num = chunk['num']
